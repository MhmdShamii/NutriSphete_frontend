...
import { getSavedMealsApi, type ProfileMeal } from "../../services/meals/mealsApis"
...
export default function SavedMeals() {
    ...
    const fetchMeals = useCallback(async (cursor: string | undefined, replace: boolean) => {
        ...
        setter(true)
        try {
            const res = await getSavedMealsApi(cursor)
    ...
    if (meals.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-16 gap-3 mt-3 rounded-2xl"
                ...
                <p className="text-sm text-text-muted">No saved meals yet</p>
            </div>
        )
    }
